        ...
        $scope.enterBorrowerId = function(event){
            if(event.which === 13){
                ...
                $http({
                    ...
                    if (response.data.statusCode == 200) {
                        var payload = response.data.payLoad
                        angular.forEach(payload, function (tuple) {
                            var loan = {}
                            loan.loanid = tuple.loanID;
                            loan.title = tuple.bookTitle;
                            loan.card_id = tuple.cardID;
                            loan.borrower_name = tuple.borrowerName;
                            loan.date_out = new Date(tuple.bookDateOut);
                            loan.due_date = new Date(tuple.bookDueDate);
                            $scope.loans.push(loan);
                        })
                    } else {
                        $mdDialog.show(
